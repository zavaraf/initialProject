...
//		String query = "select username, pass as password from usuarios where username = '"+username+"'";
	...
	public ArrayList<String> getUserRoles(String username) {
		String query = "select roles.nombreRol as role "
					+" from usuarios_has_roles ushas "
					+" join roles on roles.idRoles = ushas.Roles_idRoles " 
					+" where ushas.Usuarios_userName =  '"+username+"'";

//		List roles1 = namedParameterJdbcTemplate.queryForList(sql, getSqlParameterSource(username, ""), String.class);
